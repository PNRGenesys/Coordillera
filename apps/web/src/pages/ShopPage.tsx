...
import { type CatalogAvailability, type CatalogFilters, useGetCategoriesQuery, useGetCollectionsQuery, useGetProductsQuery } from '../store/catalog-api'
...
export function ShopPage() {
  ...
  const { data: collections } = useGetCollectionsQuery(undefined, { skip: !collection })
  ...
  const colorOptions = useMemo(() => Array.from(new Set(data?.items.flatMap((item) => item.colors) ?? [])).sort(), [data])
  const sizeOptions = useMemo(() => Array.from(new Set(data?.items.flatMap((item) => item.sizes) ?? [])).sort(), [data])
